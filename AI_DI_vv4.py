...
with _top_tabs[0]:
    ...
    if _breach_full.empty:
        st.success("No SLA breaches in the selected period.")
    else:
        st.metric("Breached Jira", f"{len(_breach_full):,}")
        _breach_display = _breach_full[[
            "ticket", "status", "Priority", "Queue", "current_owner",
            "Breach Duration", "SLA Hours", "Target Hours", "Breach Reason"
        ]].copy()
        _breach_display["ticket"] = _breach_display["ticket"].astype(str).apply(transition_dashboard_url)
        _breach_display["SLA Hours"] = _breach_display["SLA Hours"].apply(format_hours_value)
        _breach_display["Target Hours"] = _breach_display["Target Hours"].apply(format_hours_value)
        _breach_display = _breach_display.rename(columns={
