...
def build_consolidated_dataset(summary, transitions, start=None, end=None, progress=None):
    """
    ...
    if start is not None or end is not None:
        ...
        if progress is not None:
            progress(f"{len(summary):,} Jira in selected date window")
        if summary.empty:
            return pd.DataFrame()

    if st.session_state.get("overview_live_jira", False):
        ticket_keys = tuple(sorted(
            str(k).strip().upper() for k in summary.get("ticket", pd.Series(dtype=str)).dropna()
        ))
        df = refresh_current_jira_fields_cached(summary.copy(), ticket_keys, _progress=progress)
        if progress is not None:
            progress("Jira refresh done; consolidating dataset")
    else:
        ...
        df = summary.copy()
        if progress is not None:
            progress("live Jira refresh OFF for overview; using DB values")
    ...
    if "reporter" in df.columns:
        df["current_role"] = df.apply(
            lambda row: canonical_person_role(
                ...
                reporter=row.get("reporter"),
            ) or str(row.get("current_role") or ""),
            axis=1,
        )
...
overview = build_consolidated_dataset(
    ticket_summary, transitions, start=overview_start, end=overview_end, progress=_progress
)
...
health_table["_sev"] = health_table["Priority"].apply(_severity_bucket)
...
# ------------------------------------------------------------
# ROW 2 — STATUS / TRANSITION / OWNERSHIP
# ------------------------------------------------------------
left, middle, right = st.columns([1.05, 1.2, 1.05])

with left:
    st.subheader("Time Spent in Each Status (Average)")
    status_time = overview.groupby("Queue", as_index=False)["SLA Hours"].mean().sort_values("SLA Hours", ascending=True)
    status_time["Label"] = status_time["SLA Hours"].apply(lambda x: format_hours_value(x))
    fig = px.bar(status_time, x="SLA Hours", y="Queue", orientation="h", text="Label")
    fig.update_traces(textposition="outside")
    fig.update_layout(height=300, margin=dict(l=10, r=60, t=10, b=10), xaxis_title="Hours", yaxis_title="")
    st.plotly_chart(fig, width="stretch", key="overview_status_time")

with middle:
    st.subheader("Transition SLA (Average | P90)")
    transition_rows = []
    for queue, group in overview.groupby("Queue"):
        vals = group["SLA Hours"]
        transition_rows.append({
            "Transition": queue,
            "Avg Time": format_hours_value(vals.mean()),
            "P90 Time": format_hours_value(vals.quantile(.90)),
...
with right:
    ...
    ownership = pd.DataFrame({
        "Category": ["L3", "Developers", "Other (counted)", "Customer Wait (paused)"],
        "Hours": [
...
# ROW 3 — AGING + OWNERS
# ------------------------------------------------------------
left, middle, right = st.columns([1.0, 1.15, 1.15])
...
with middle:
    ...
    if l3.empty:
        ...
        owner_table["SLA %"] = (owner_table["SLA_Compliance"] * 100).round(0).astype(int).astype(str) + "%"
...
# ------------------------------------------------------------
# ROW 4 — HANDOFFS + RECENT BREACHES
# ------------------------------------------------------------
...
with right:
    ...
    recent = overview[overview["SLA Breached"]].copy().sort_values("SLA Hours", ascending=False).head(10)
    if recent.empty:
        st.success("No SLA breaches in the selected period.")
    else:
        recent_display = recent[["ticket", "status", "Priority", "Queue", "current_owner", "Breach Duration", "SLA Hours", "Target Hours", "Breach Reason"]].copy()
        recent_display["ticket"] = recent_display["ticket"].astype(str).apply(transition_dashboard_url)
        recent_display["SLA Hours"] = recent_display["SLA Hours"].apply(format_hours_value)
        recent_display["Target Hours"] = recent_display["Target Hours"].apply(format_hours_value)
        recent_display = recent_display.rename(columns={
            "ticket":"Jira", "status":"Status", "Priority":"Priority", "Queue":"Breached At", "current_owner":"Owner", "Breach Duration":"Time Breached", "SLA Hours":"Total SLA", "Target Hours":"SLA Target", "Breach Reason":"Reason"
        })
        st.dataframe(
            recent_display,
            width="stretch",
